...
export async function updateSession(request: NextRequest) {
  ...
  // supabase1.auth.getUser(). A simple mistake could make it very hard to debug
  ...
  // Check for session error or expired session
  if (error?.status === 401 || !user) {
    // Sign out the user
    await supabase.auth.signOut();

    // Don't redirect if already on these paths
    if (
